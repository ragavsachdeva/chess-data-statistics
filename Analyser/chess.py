# -*- coding: utf-8 -*-
# This file largely contains code that is part of the python-chess library:
#       Copyright (C) 2012-2017 Niklas Fiekas <niklas.fiekas@backscattering.de>
# Game analysis functionality has been added by Ragav Sachdeva
# You are free to use this code as long as proper credit is given to the authors
...
class Board(BaseBoard):
	...
	def __init__(self, fen=STARTING_FEN, chess960=False):
		...
		self.totalCaptured = 0

		self.moveDistribution = {
		'a1':{}, 'b1':{}, 'c1':{}, 'd1':{}, 'e1':{}, 'f1':{}, 'g1': {}, 'h1': {},
		'a2':{}, 'b2':{}, 'c2':{}, 'd2':{}, 'e2':{}, 'f2':{}, 'g2': {}, 'h2': {},
		'a7':{}, 'b7':{}, 'c7':{}, 'd7':{}, 'e7':{}, 'f7':{}, 'g7': {}, 'h7': {},
		...
		self.openings = {"name": "ref", "children": []}
	...
	def simulate_move(self,move_pgn,_from,_to,wasPromoted):
		if move_pgn == "O-O" or move_pgn == "O-O+" or move_pgn == "O-O#":
			self.currentBoardState[_to] = self.currentBoardState[_from]
			self.currentBoardState[_from] = ""
			#king-side castelling
			if _from == "e1":
				self.currentBoardState["h1"] = ""
				self.currentBoardState["f1"] = "Rh1"
			if _from == "e8":
				self.currentBoardState["h8"] = ""
				self.currentBoardState["f8"] = "Rh8"

		elif move_pgn == "O-O-O" or move_pgn == "O-O-O+" or move_pgn == "O-O-O#":
			self.currentBoardState[_to] = self.currentBoardState[_from]
			self.currentBoardState[_from] = ""
			#queen-side castelling
			if _from == "e1":
				self.currentBoardState["a1"] = ""
				self.currentBoardState["d1"] = "Ra1"
			if _from == "e8":
				self.currentBoardState["a8"] = ""
				self.currentBoardState["d8"] = "Ra8"

		else:
			if wasPromoted and self.currentBoardState[_from]!="PromotedPiece":
				self.currentBoardState[_from] = "PromotedPiece"

			self.currentBoardState[_to] = self.currentBoardState[_from]
			self.currentBoardState[_from] = ""


	def pieceColour(self,piecepos):
		piecesan = self.currentBoardState[piecepos]
		if piecesan == "Ra1" or piecesan == "Nb1" or piecesan == "Bc1" or piecesan == "Qd1" or piecesan == "Ke1" or piecesan == "Bf1" or piecesan == "Ng1" or piecesan == "Rh1" or piecesan=="a2" or piecesan == "b2" or piecesan=="c2" or piecesan=="d2" or piecesan=="e2" or piecesan=="f2" or piecesan=="g2" or piecesan=="h2":
			return "white"
		else:
			return "black"

	def first_move_distribution(self,san,moveNumber):
		#first move distribution of every piece
		...
		if move_pgn == "O-O" or move_pgn == "O-O+" or move_pgn == "O-O#":
			#king side castelling
			#definitely a first move so no need need to check
			#and now they have moved so change their state
			self.hasNotMovedYet[_from] = False
			file = _from[1:]
			self.hasNotMovedYet["h"+file] = False

			#if the move number exists for the king
			if str(moveNumber) in self.moveDistribution[_from]:
				#then add 1
				self.moveDistribution[_from][str(moveNumber)] += 1
			else:
				#initialise the king's number to 1
				self.moveDistribution[_from][str(moveNumber)] = 1
			
			#if move number of the rook exists
			if str(moveNumber) in self.moveDistribution["h"+file]:
				#then add 1
				self.moveDistribution["h"+file][str(moveNumber)] += 1
			else:
				#initialise to rook's number 1
				self.moveDistribution["h"+file][str(moveNumber)] = 1

		elif move_pgn == "O-O-O" or move_pgn == "O-O-O+" or move_pgn == "O-O-O#":
			#queen side castelling
			#definitely a first move so need need to check
			#and now they have moved so change their state
			self.hasNotMovedYet[_from] = False
			file = _from[1:]
			self.hasNotMovedYet["h"+file] = False
			...
			#if move number of the rook exists
			if str(moveNumber) in self.moveDistribution["a"+file]:
				#then add 1
				self.moveDistribution["a"+file][str(moveNumber)] += 1
			else:
				#initialise to rook's number 1
				self.moveDistribution["a"+file][str(moveNumber)] = 1

		else:
			#checking if the square belongs to a piece and if it hasn't moved yet
			if _from in self.hasNotMovedYet and self.hasNotMovedYet[_from]:
				#change the state
				self.hasNotMovedYet[_from] = False
				#if the move number exists for this piece
				if str(moveNumber) in self.moveDistribution[_from]:
					#add 1
					self.moveDistribution[_from][str(moveNumber)] += 1
				else:
					#initialise the move number to 1
					self.moveDistribution[_from][str(moveNumber)] = 1

		return move

	def captured_squares_frequency(self,san):
		move = self.parse_san(san)
		wasPromoted = self.push(move)
		...
		if move_pgn == "O-O" or move_pgn == "O-O+" or move_pgn == "O-O#":
			...
			if self.currentBoardState[_to]!="":
				if self.currentBoardState[_to] == "PromotedPiece":
					pass
				else:
					self.capturedSquares[self.currentBoardState[_to]][_to] += 1
				
				self.capturedSquares["all"][_to] += 1
				if(self.pieceColour(_to) == "white"):
					self.capturedSquares["wall"][_to] += 1
				else:
					self.capturedSquares["ball"][_to] += 1
			if wasPromoted and self.currentBoardState[_from]!="PromotedPiece":
				self.currentBoardState[_from] = "PromotedPiece"
	...
	def capturing_squares_frequency(self,san):
		move = self.parse_san(san)
		wasPromoted = self.push(move)
		...
		if move_pgn == "O-O" or move_pgn == "O-O+" or move_pgn == "O-O#":
			...
			if self.currentBoardState[_to]!="":
				if self.currentBoardState[_from] == "PromotedPiece":
					pass
				else:
					self.capturingSquares[self.currentBoardState[_from]][_to] += 1
				self.capturingSquares["all"][_to] += 1
				if(self.pieceColour(_from) == "white"):
					self.capturingSquares["wall"][_to] += 1
				else:
					self.capturingSquares["ball"][_to] += 1
			if wasPromoted and self.currentBoardState[_from]!="PromotedPiece":
				self.currentBoardState[_from] = "PromotedPiece"
	...
	def path_trajectory(self,san):
		...
		if move_pgn == "O-O" or move_pgn == "O-O+" or move_pgn == "O-O#":
			...
			if str(_from+"-"+_to) in self.piecePath[self.currentBoardState[_from]]:
				self.piecePath[self.currentBoardState[_from]][_from+"-"+_to] += 1
			else:
				self.piecePath[self.currentBoardState[_from]][_from+"-"+_to] = 1
			...
			self.currentBoardState[_from] = ""
			if _from == "e1":
				if str("h1"+"-"+"f1") in self.piecePath["Rh1"]:
					self.piecePath["Rh1"]["h1"+"-"+"f1"] += 1
				else:
					self.piecePath["Rh1"]["h1"+"-"+"f1"] = 1
				self.currentBoardState["f1"] = self.currentBoardState["h1"]
				self.currentBoardState["h1"] = ""
			if _from == "e8":
				if str("h8"+"-"+"f8") in self.piecePath["Rh8"]:
					self.piecePath["Rh8"]["h8"+"-"+"f8"] += 1
				else:
					self.piecePath["Rh8"]["h8"+"-"+"f8"] = 1
				self.currentBoardState["f8"] = self.currentBoardState["h8"]
				self.currentBoardState["h8"] = ""
			...
			self.currentBoardState[_from] = ""
			if _from == "e1":
				if str("a1"+"-"+"d1") in self.piecePath["Ra1"]:
					self.piecePath["Ra1"]["a1"+"-"+"d1"] += 1
				else:
					self.piecePath["Ra1"]["a1"+"-"+"d1"] = 1
				self.currentBoardState["d1"] = self.currentBoardState["a1"]
				self.currentBoardState["a1"] = ""
			if _from == "e8":
				if str("a8"+"-"+"d8") in self.piecePath["Ra8"]:
					self.piecePath["Ra8"]["a8"+"-"+"d8"] += 1
				else:
					self.piecePath["Ra8"]["a8"+"-"+"d8"] = 1
				self.currentBoardState["d8"] = self.currentBoardState["a8"]
				self.currentBoardState["a8"] = ""

		elif wasPromoted and self.currentBoardState[_from]!="PromotedPiece":
			self.currentBoardState[_to] = "PromotedPiece"
			self.currentBoardState[_from] = ""
			...
			elif str(_from+"-"+_to) in self.piecePath[self.currentBoardState[_from]]:
				self.piecePath[self.currentBoardState[_from]][_from+"-"+_to] += 1
	...
	def square_utilisation(self,san):
		move = self.parse_san(san)
		wasPromoted = self.push(move)
		...
		if move_pgn == "O-O" or move_pgn == "O-O+" or move_pgn == "O-O#":
			...
			if self.currentBoardState[_from] == "PromotedPiece":
				...
				self.squareUtilisation[self.currentBoardState[_from]][_to] += 1
			
			self.squareUtilisation["all"][_to] += 1
			
			if(self.pieceColour(_from) == "white"):
				self.squareUtilisation["wall"][_to] += 1
			else:
				self.squareUtilisation["ball"][_to] += 1
	...
	def compute_openings(self,san):
		move1 = self.parse_san(san[0])
		wasPromoted1 = self.push(move1)

		given_move1 = str(move1)
		_from1 = given_move1[:2]
		_to1 = given_move1[2:4]
		move_pgn1 = str(san[0])
		existed1 = False
		dmove1 = _from1+"-"+_to1
		index1 = 0
		
		for piece in self.openings["children"]:
			if piece["name"] ==  move_pgn1:
				piece["move"] = dmove1
				existed1 = True
				break
			index1 += 1
		...
		if not existed1:
			child = {"name":move_pgn1, "move":dmove1, "children": []}
			self.openings["children"].append(child)

		self.simulate_move(move_pgn1,_from1,_to1,wasPromoted1)

		move2 = self.parse_san(san[1])
		wasPromoted2 = self.push(move2)

		given_move2 = str(move2)
		_from2 = given_move2[:2]
		_to2 = given_move2[2:4]
		move_pgn2 = str(san[1])
		existed2 = False
		dmove2 = _from2+"-"+_to2
		index2 = 0

		for piece in self.openings["children"][index1]["children"]:
			if piece["name"] ==  move_pgn2:
				piece["move"] = dmove2
				existed2 = True
				break
			index2 += 1
		...
		if not existed2:
			child = {"name":move_pgn2,"move":dmove2,"children": []}
			self.openings["children"][index1]["children"].append(child)

		self.simulate_move(move_pgn2,_from2,_to2,wasPromoted2)

		move3 = self.parse_san(san[2])
		wasPromoted3 = self.push(move3)

		given_move3 = str(move3)
		_from3 = given_move3[:2]
		_to3 = given_move3[2:4]
		move_pgn3 = str(san[2])
		existed3 = False
		dmove3 = _from3+"-"+_to3
		index3 = 0

		for piece in self.openings["children"][index1]["children"][index2]["children"]:
			if piece["name"] ==  move_pgn3:
				existed3 = True
				piece["move"] = dmove3
				break
			index3 += 1
		...
		if not existed3:
			child = {"name":move_pgn3, "move":dmove3, "children": []}
			self.openings["children"][index1]["children"][index2]["children"].append(child)

		self.simulate_move(move_pgn3,_from3,_to3,wasPromoted3)

		move4 = self.parse_san(san[3])
		wasPromoted4 = self.push(move4)

		given_move4 = str(move4)
		_from4 = given_move4[:2]
		_to4 = given_move4[2:4]
		move_pgn4 = str(san[3])
		existed4 = False
		dmove4 = _from4+"-"+_to4
		index4 = 0

		for piece in self.openings["children"][index1]["children"][index2]["children"][index3]["children"]:
			if piece["name"] ==  move_pgn4:
				existed4 = True
				piece["move"] = dmove4
				break
			index4 += 1
		...
		if not existed4:
			child = {"name":move_pgn4,"move":dmove4, "children": []}
			self.openings["children"][index1]["children"][index2]["children"][index3]["children"].append(child)

		self.simulate_move(move_pgn4,_from4,_to4,wasPromoted4)

		move5 = self.parse_san(san[4])
		wasPromoted5 = self.push(move5)

		given_move5 = str(move5)
		_from5 = given_move5[:2]
		_to5 = given_move5[2:4]
		move_pgn5 = str(san[4])
		existed5 = False
		dmove5 = _from5+"-"+_to5
		index5 = 0

		for piece in self.openings["children"][index1]["children"][index2]["children"][index3]["children"][index4]["children"]:
			if piece["name"] ==  move_pgn5:
				existed5 = True
				piece["move"] = dmove5
				break
			index5 += 1
		...
		if not existed5:
			child = {"name":move_pgn5,"move":dmove5, "children": []}
			self.openings["children"][index1]["children"][index2]["children"][index3]["children"][index4]["children"].append(child)

		self.simulate_move(move_pgn5,_from5,_to5,wasPromoted5)

		move6 = self.parse_san(san[5])
		wasPromoted6 = self.push(move6)

		given_move6 = str(move6)
		_from6 = given_move6[:2]
		_to6 = given_move6[2:4]
		move_pgn6 = str(san[5])
		existed6 = False
		dmove6 = _from6+"-"+_to6
		index6 = 0

		for piece in self.openings["children"][index1]["children"][index2]["children"][index3]["children"][index4]["children"][index5]["children"]:
			if piece["name"] ==  move_pgn6:
				existed6 = True
				piece["move"] = dmove6
				break
			index6 += 1
		...
		if not existed6:
			child = {"name":move_pgn6,"move":dmove6, "children": []}
			self.openings["children"][index1]["children"][index2]["children"][index3]["children"][index4]["children"][index5]["children"].append(child)

		self.simulate_move(move_pgn6,_from6,_to6,wasPromoted6)

		move7 = self.parse_san(san[6])
		wasPromoted7 = self.push(move7)

		given_move7 = str(move7)
		_from7 = given_move7[:2]
		_to7 = given_move7[2:4]
		move_pgn7 = str(san[6])
		existed7 = False
		dmove7 = _from7+"-"+_to7
		index7 = 0

		for piece in self.openings["children"][index1]["children"][index2]["children"][index3]["children"][index4]["children"][index5]["children"][index6]["children"]:
			if piece["name"] ==  move_pgn7:
				piece["size"] += 1
				piece["move"] = dmove7
				existed = True
				break
		...
		if not existed7:
			child = {"name":move_pgn7, "move":dmove7, "size": 1}
			self.openings["children"][index1]["children"][index2]["children"][index3]["children"][index4]["children"][index5]["children"][index6]["children"].append(child)

		self.simulate_move(move_pgn7,_from7,_to7,wasPromoted7)
