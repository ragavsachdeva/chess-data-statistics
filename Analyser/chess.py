...
class Board(BaseBoard):
	...
	def captured_squares_frequency(self,san):
		...
		_to = given_move[2:4]
		move_pgn = str(san)
		self.chessBoard[_from] = False

		if move_pgn == "O-O" or move_pgn == "O-O+" or move_pgn == "O-O#":
			#king-side castelling
			if _from == "e1":
				self.chessBoard["h1"] = False
				self.chessBoard["f1"] = True
			if _from == "e8":
				self.chessBoard["h8"] = False
				self.chessBoard["f8"] = True

		elif move_pgn == "O-O-O" or move_pgn == "O-O-O+" or move_pgn == "O-O-O#":
			#queen-side castelling
			if _from == "e1":
				self.chessBoard["a1"] = False
				self.chessBoard["d1"] = True
			if _from == "e8":
				self.chessBoard["a8"] = False
				self.chessBoard["d8"] = True

		else:
			if self.chessBoard[_to]:
				self.capturedSquares[_to] += 1
				self.totalCaptured += 1
		self.chessBoard[_to] = True
		return move
